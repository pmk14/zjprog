    ...
    def update_quality(items):
        for i in range(0, len(items)):
            if "Aged Brie" != items[i].name and "Backstage passes to a TAFKAL80ETC concert" != items[i].name:
                ...
                if items[i].quality > 0 and "Sulfuras, Hand of Ragnaros" != items[i].name:
                ...
                if items[i].quality < 50:
                    ...
                    if "Backstage passes to a TAFKAL80ETC concert" == items[i].name:
                        if items[i].sell_in < 11 and items[i].quality < 50:
                                items[i].quality = items[i].quality + 1
                        # Increases the Quality of Backstage Passes if the Quality is 6 or less.
                        if items[i].sell_in < 6 and items[i].quality < 50:
                                items[i].quality = items[i].quality + 1
